...
function parseReviewsCsv(text) {
  ...
  // En-têtes : trim des espaces + gestion doublons Évaluation/évaluation
  const rawHeaders = parseCsvLine(firstLine, sep).map(h => h.trim())
  const seenEval = { upper: false, lower: false }
  const fieldMap = rawHeaders.map(h => {
    // Distinction majuscule/minuscule pour "Évaluation" vs "évaluation"
    if (h === 'Évaluation' || h === '\u00c9valuation') {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (row.rating) row.rating = parseInt(row.rating, 10) || null
